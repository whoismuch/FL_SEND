#!/bin/bash
#SBATCH --job-name=fl_send_cpu_10k_b8
#SBATCH --time=10-00:00:00  # 10 days (should be enough for 10000 samples on CPU)
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8  # For 2 clients × 4 CPUs each
#SBATCH --mem=32G  # Reduced memory for CPU mode (sequence length is fixed at 1000 frames)
...
#SBATCH --output=fl_training_cpu_10000_samples_batch8_%j.out
#SBATCH --error=fl_training_cpu_10000_samples_batch8_%j.err
...
# Lower memory threshold from 0.95 to 0.85 to be more conservative for CPU
...
if [ -n "$CONDA_ENV_PATH" ] && [ -f "$CONDA_ENV_PATH/bin/python" ]; then
    ...
    echo "ERROR: Environment flsend_clean not found on CPU node!"
...
echo "=== Starting CPU Federated Learning Training on 10000 SAMPLES ==="
echo "Configuration:"
...
LOG_FILE="logs/fl_training_cpu_10000_samples_batch8_$(date +%Y%m%d_%H%M%S).log"
...
# Run federated learning training and save all output to log file
...
#   --batch_size 4: Reduced batch size for CPU (from 8 to 4 for better memory usage)
