#!/bin/bash
#SBATCH --job-name=fl_send_training_full_b8
#SBATCH --time=14-00:00:00  # 14 days (should be enough for full dataset)
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --output=fl_training_full_dataset_gpu_batch8_%j.out
#SBATCH --error=fl_training_full_dataset_gpu_batch8_%j.err
...
echo "=== Starting Federated Learning Training on FULL DATASET ==="
...
LOG_FILE="logs/fl_training_full_dataset_gpu_batch8_$(date +%Y%m%d_%H%M%S).log"
...
# No --test_size argument means using ALL available data
...
#   --num_rounds 20: Number of federated learning rounds (more rounds for full dataset)
