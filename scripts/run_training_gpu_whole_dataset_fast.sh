#!/bin/bash
#SBATCH --job-name=send_training_fast
...
#SBATCH --output=training_fast_%j.out
#SBATCH --error=training_fast_%j.err
...
echo "=== Starting FAST Training on FULL DATASET with 100 EPOCHS ==="
echo "=== OPTIMIZATIONS: DER computation DISABLED during training (faster) ==="
...
PYTHONUNBUFFERED=1 python src/SEND_PSE_AMI.py --epochs 100 --chunk_size 250 --batch_size 2 --max_sequence_length 1000
